...
- **Transactional consistency guard** - A strongly authenticated invoice, statement, security code, or new-device notice whose claimed identity matches the sender, with no list headers, warning signal, or foreign target, cannot be pushed into junk by an unstable AI verdict alone. The guard gives no Ham credit and never rescues Rspamd's own score: it only caps the AI-added part just below the junk threshold, and every hit is logged and reported for review.
